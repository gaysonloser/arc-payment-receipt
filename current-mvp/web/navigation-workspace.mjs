...
function a12UiStyles() {
  ...
  style.textContent = `
    ...
    .a12-guide-blocker{margin-top:6px!important;color:var(--a12-text)!important}.a12-document-controls{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid var(--a12-border)}.a12-document-controls>div:first-child{display:grid;gap:4px;min-width:0}.a12-document-state{color:var(--a12-muted);font-size:12px;font-weight:750}.a12-document-blocker{overflow-wrap:anywhere;color:var(--a12-text);font-size:12px}.a12-document-nav{display:flex;flex-wrap:wrap;gap:6px}.a12-document-nav button{min-height:36px;padding:6px 9px;border:1px solid var(--a12-border);border-radius:5px;background:#fff;color:var(--a12-text);font-size:12px;font-weight:750}
    ...
    .a12-reconciliation-scroll{max-width:100%;overflow-x:auto;overscroll-behavior-inline:contain;scrollbar-gutter:stable}.a12-reconciliation-table td{font-size:12px}.a12-reconciliation-table tr[data-first-failure="true"]{box-shadow:inset 4px 0 0 var(--a12-danger)}.a12-row-action{min-height:44px;padding:7px 9px;border:1px solid var(--a12-focus);border-radius:6px;background:#fff;color:var(--a12-focus);font-size:12px;font-weight:800;text-align:left}.a12-row-state{display:inline-flex;min-height:44px;align-items:center;color:var(--a12-muted);font-size:12px}.a12-reconciliation-consequence{display:grid;grid-template-columns:minmax(220px,.8fr) minmax(320px,1.3fr) auto;gap:14px;align-items:center;margin-top:14px;padding:14px;border:1px solid var(--a12-border);border-left:4px solid var(--a12-warning);border-radius:8px;background:#fbfaf5}.a12-reconciliation-consequence[data-matcher-state="matched"]{border-left-color:var(--a12-success);background:#f3faf6}.a12-reconciliation-consequence h3{margin:3px 0 4px;font-size:15px}.a12-reconciliation-consequence p{margin:0;color:var(--a12-muted);font-size:12px}.a12-reconciliation-consequence dl{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin:0}.a12-reconciliation-consequence dl>div{min-width:0;padding:8px;border:1px solid var(--a12-border);background:#fff}.a12-reconciliation-consequence dt{color:var(--a12-muted);font-size:12px;font-weight:750}.a12-reconciliation-consequence dd{margin:3px 0 0;overflow-wrap:anywhere;font-size:12px;font-weight:750}.a12-reconciliation-consequence button{max-width:190px;white-space:normal}
    ...
    @media(max-width:1100px){.a12-reconciliation-consequence{grid-template-columns:1fr}.a12-reconciliation-consequence button{max-width:none;width:100%}}
    @media(max-width:900px){.a12-workspace-grid{grid-template-columns:1fr}.a12-workspace-guide{grid-template-columns:52px minmax(0,1fr)}.a12-workspace-guide img{width:52px;height:52px}.a12-reconciliation-table{min-width:760px}.a12-reconciliation-scroll+ .a12-boundary{margin-top:12px}}
    ...
    @media(min-width:1311px) and (max-width:1450px){.a12-shell{grid-template-columns:minmax(104px,1fr) repeat(6,max-content);gap:5px 6px;padding-left:12px;padding-right:12px}.a12-shell-status{padding-left:5px;padding-right:5px;font-size:12px}}
    .a12-count,.a12-direction,.a12-queue-row small,.a12-queue-meta,.a12-queue-meta strong,.a12-filter-row button,.a12-panel-head p{font-size:12px}
    ...
    /* The inspector is a drawer below 1051px, so it must not reserve a third grid column. */
    @media(min-width:721px) and (max-width:1050px){.a12-workbench{grid-template-columns:248px minmax(0,1fr)}}
    /* Operational text floor: interactive labels, state, recovery, provenance and live notices always win at 12px or above. */
    .a12-shell-status,.a12-state-pill,.a12-object-ref button,.a12-origin-entry-option small,.a12-live,.a12-bottom-action .a12-kicker,.a12-field-hint,.a12-disclosure>summary span,.a12-status-meta,.a12-receipt-field b,.a12-receipt-field span,.a12-object-row b,.a12-object-row span,.a12-object-row small,.a12-causal-stage strong,.a12-causal-stage span,.a12-causal-stage small{font-size:12px!important}
...
export function a12ReconciliationRows(view, state) {
  if (!state?.evidence) {
    return view.inspector.receiptFields.slice(0, 5).map((field, index) => ({
      field: field.fieldId,
      locked: a12ReconciliationValue(field.value),
      observed: "missing",
      status: index === 0 ? "blocking" : "held",
      source: a12UiSource(field.source),
      recoveryAction: index === 0 ? "Open Arc evidence" : "Await first blocker",
      actionTab: "Arc",
      actionable: index === 0
    }));
  }
  ...
  for (const record of state.evidence.ordered_records ?? []) {
    for (const field of Object.keys(record.expected ?? {})) {
      ...
      const status = locked === observed ? "matched" : "blocking";
      rows.push({ field: `${record.type}.${field}`, locked, observed, status, source: record.source, recoveryAction: status === "blocking" ? `Inspect ${record.type}.${field}` : "Verified", actionTab: "Arc", actionable: status === "blocking" });
    }
  }
  if (state.matcherState === "stale") {
    rows.unshift({ field: "receipt_freshness", locked: `within TTL · confirmations ≥ ${state.evidence.finality?.finality_threshold ?? "required"}`, observed: `stale · confirmations ${state.evidence.finality?.confirmations ?? "missing"}`, status: "blocking", source: "typed finality observation", recoveryAction: "Refresh receipt TTL", actionTab: "Arc", actionable: true });
  }
...
export function a12WorkspacePrimaryMarkup(view, state, queue) {
  ...
  const lastCheckedField = view.inspector.receiptFields.find((field) => field.fieldId === "observation_timestamp");
  const lastChecked = lastCheckedField ? a12UiPublicReceiptValue(lastCheckedField.value) : "not observed";
  const documentHeader = `<div class="a12-document-controls"><div><span class="a12-document-state">OPEN · Matcher: ${a12UiEscape(view.canvas.matcherState)} · last checked ${a12UiEscape(lastChecked)}</span><span class="a12-document-blocker"><b>Gayson · First blocker:</b> ${a12UiEscape(view.canvas.firstFailure)} · <b>Recovery:</b> ${a12UiEscape(view.canvas.recovery)}</span></div><div class="a12-document-nav" aria-label="Accounting document navigation"><button type="button" data-a12-prev-document aria-label="Previous accounting document">← Previous</button><button type="button" data-a12-next-document aria-label="Next accounting document">Next →</button><button type="button" data-a12-open-inspector aria-label="Open document history and evidence">History / evidence</button></div></div>`;
  const guide = `<div class="a12-workspace-guide"><img src="assets/gayson-receipt-assistant-reference.png" alt="Gayson task assistant"><div><span class="a12-kicker">Gayson · ${a12UiEscape(workspace.title)}</span><h2>${a12UiEscape(workspace.primaryJob)}</h2><p>${a12UiEscape(workspace.detail)}</p><p class="a12-guide-blocker"><b>First blocker:</b> ${a12UiEscape(view.canvas.firstFailure)} · <b>Recovery:</b> ${a12UiEscape(view.canvas.recovery)}</p></div></div>`;
  let body = "";
  if (workspace.id === "payables" || workspace.id === "receivables") {
    ...
    const firstBlockingField = comparisonRows.find((row) => row.status === "blocking")?.field ?? "none";
    const consequenceReady = state.matcherState === "matched";
    body = `<div id="reconciliation-workbench-root" tabindex="-1"><div class="a12-reconciliation-scroll" role="region" aria-label="Scrollable locked document and receipt comparison" tabindex="0"><table class="a12-workspace-table a12-reconciliation-table" aria-label="Locked document versus typed receipt comparison"><thead><tr><th>Field</th><th>Locked</th><th>Readback</th><th>Status / source</th><th>Recovery</th></tr></thead><tbody>${comparisonRows.map((field) => `<tr data-first-failure="${String(field.field === firstBlockingField)}" data-selected="${String(field.status === "blocking")}"><td><b>${a12UiEscape(field.field)}</b></td><td>${a12UiEscape(field.locked)}</td><td>${a12UiEscape(field.observed)}</td><td>${a12UiEscape(field.status)}<br><small>${a12UiEscape(field.source)}</small></td><td>${field.actionable ? `<button type="button" class="a12-row-action" data-a12-reconciliation-action data-a12-reconciliation-field="${a12UiEscape(field.field)}" data-a12-reconciliation-tab="${a12UiEscape(field.actionTab)}" aria-label="${a12UiEscape(field.recoveryAction)} for ${a12UiEscape(field.field)}">${a12UiEscape(field.recoveryAction)}</button>` : `<span class="a12-row-state">${a12UiEscape(field.recoveryAction)}</span>`}</td></tr>`).join("")}</tbody></table></div><div class="a12-boundary"><b>First exact blocker</b>${a12UiEscape(firstBlockingField)} · ${a12UiEscape(view.canvas.recovery)}</div><section class="a12-reconciliation-consequence" data-a12-reconciliation-consequence data-matcher-state="${a12UiEscape(state.matcherState)}" aria-label="Bounded ERP consequence"><div><span class="a12-kicker">ERP consequence · read-only</span><h3>${consequenceReady ? "Receipt matched; journal proposal may be reviewed" : "Receipt is not matched; ERP proposal stays held"}</h3><p>Chain evidence, ERP posting and business close remain separate owner-controlled facts.</p></div><dl><div><dt>Payment Entry</dt><dd>${a12UiEscape(view.inspector.consequences.paymentEntry)}</dd></div><div><dt>Bank Transaction</dt><dd>${a12UiEscape(view.inspector.consequences.bankTransaction)}</dd></div><div><dt>GL / PLED</dt><dd>${a12UiEscape(view.inspector.consequences.glPled)}</dd></div><div><dt>Outstanding / close</dt><dd>${a12UiEscape(view.inspector.consequences.outstanding)} · ${a12UiEscape(view.inspector.consequences.close)}</dd></div></dl><button type="button" class="a12-secondary" data-a12-workspace="general-ledger" ${consequenceReady ? "" : "disabled aria-disabled=\"true\""}>Review non-posting journal proposal</button></section></div>`;
  ...
  return `<section class="a12-workspace-primary" data-a12-workspace-primary="${a12UiEscape(workspace.id)}" aria-label="${a12UiEscape(workspace.title)} primary workspace">${documentHeader}${guide}${body}</section>`;
}
...
export function mountA12DeepWorkbench(root = null, { initialHash = globalThis.location?.hash ?? "" } = {}) {
  ...
  host.addEventListener("click", (event) => {
    const target = event.target.closest("[data-a12-workspace],[data-a12-scenario],[data-a12-stage],[data-a12-tab],[data-a12-filter],[data-a12-primary],[data-a12-milestone-primary],[data-a12-milestone-outcome],[data-a12-open-inspector],[data-a12-close-inspector],[data-a12-object],[data-a12-edit-select],[data-a12-origin-entry],[data-a12-reconciliation-action],[data-a12-prev-document],[data-a12-next-document]");
    ...
    if (target.hasAttribute("data-a12-prev-document")) return act({ type: "QUEUE_MOVE", direction: "up" }, "#a12-task-heading");
    if (target.hasAttribute("data-a12-next-document")) return act({ type: "QUEUE_MOVE", direction: "down" }, "#a12-task-heading");
    ...
    if (target.hasAttribute("data-a12-reconciliation-action")) {
      state = reduceA12Workbench(state, { type: "SET_INSPECTOR_TAB", tab: target.dataset.a12ReconciliationTab || "Arc" });
      state.lastNotice = `${target.dataset.a12ReconciliationField} opened as the exact reconciliation blocker; no ERP mutation or business close was performed.`;
      return render("#a12-evidence-inspector");
    }
