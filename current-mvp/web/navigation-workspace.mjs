...
function a12UiStyles() {
  ...
  style.textContent = `
    ...
    .a12-milestone-document{display:grid;grid-template-columns:52px minmax(0,1fr) auto;gap:12px;align-items:center;margin:0 0 16px;padding:14px 16px;border:1px solid #b9c9d7;border-radius:8px;background:#f7fbff}.a12-milestone-document-icon{display:grid;width:52px;height:52px;place-items:center;border-radius:8px;background:#e2eefb}.a12-milestone-document-icon img{width:30px;height:30px}.a12-milestone-document-copy h3{margin:2px 0 3px;font-family:Georgia,"Times New Roman",serif;font-size:20px;font-weight:650}.a12-milestone-document-copy p{margin:0;color:var(--a12-muted);font-size:13px}.a12-milestone-document-copy p b{color:var(--a12-text)}.a12-summary-brands{display:flex;align-items:center;gap:5px}.a12-summary-brands img{width:30px!important;height:30px!important}.a12-milestone-guide-note{margin-top:2px;padding:11px 13px;border-left:3px solid #287553;background:#f1faf5;color:var(--a12-muted);font-size:12px}.a12-milestone-guide-note b{display:block;margin-bottom:3px;color:#1d5c43;font-size:13px}.a12-milestone-step{min-height:78px}.a12-milestone-step[data-a12-milestone-step="document"]{border-left:4px solid #287553}.a12-milestone-step[data-a12-milestone-step="gl-close"]{border-left:4px solid var(--a12-warning)}
    .a12-app[data-current-workspace="milestone-desk"] .a12-milestone-layout{grid-template-columns:minmax(0,1.55fr) minmax(320px,.78fr);align-items:start}.a12-app[data-current-workspace="milestone-desk"] .a12-milestone-summary{position:sticky;top:16px}.a12-app[data-current-workspace="milestone-desk"] .a12-milestone-supporting-detail{margin-top:16px}.a12-app[data-current-workspace="milestone-desk"] .a12-milestone-supporting-detail>summary{background:#fffdf8}
...
export function a12WorkspacePrimaryMarkup(view, state, queue) {
  ...
  if (workspace.id === "payables" || workspace.id === "receivables") {
    ...
    const steps = [
      ["document", "file-invoice-dollar.svg", "Document · source control", `${view.canvas.sourceDocument} · ${view.canvas.counterparty} · open item`, "verified"],
      ["reviewer", "circle-check.svg", "Reviewer · typed attestation", state.evidence ? `${state.evidence.outcome} · reviewer evidence` : "Awaiting a bound typed reviewer observation", state.evidence ? "verified" : "held"],
      ["arc-receipt", "receipt.svg", "Arc receipt · policy readback", state.evidence ? `Receipt outcome ${state.evidence.outcome} · Arc Testnet` : "No current settlement receipt observed", state.evidence ? "current" : "held"],
      ["erp-reconciliation", "scale-balanced.svg", "ERP reconciliation · Payment Entry / Bank", `${view.inspector.consequences.paymentEntry}; ${view.inspector.consequences.bankTransaction}`, "held"],
      ["gl-close", "book-open.svg", "GL close · PLED / period boundary", `${view.inspector.consequences.glPled}; books closed remains false`, "held"]
    ];
    body = `<article class="a12-milestone-document" data-testid="milestone-business-document" aria-label="Business accounting document"><div class="a12-milestone-document-icon"><img src="assets/icons/file-invoice-dollar.svg" alt=""></div><div class="a12-milestone-document-copy"><span class="a12-kicker">Accounting document center</span><h3>${a12UiEscape(view.canvas.sourceDocument)}</h3><p><b>${a12UiEscape(view.canvas.counterparty)}</b> · open item · principal ${a12UiEscape(view.canvas.principal)}</p></div><span class="a12-state-pill tone-${a12UiEscape(view.canvas.scenarioTone)}" data-testid="milestone-document-status">${a12UiEscape(view.canvas.scenarioState)}</span></article><div class="a12-milestone-layout" data-testid="milestone-decision-layout"><div class="a12-milestone-timeline" aria-label="Document to reviewer to Arc receipt to ERP reconciliation to GL close timeline">${steps.map(([stepId,icon,title,detail,status]) => `<div class="a12-milestone-step" data-a12-milestone-step="${stepId}" data-state="${status}"><img src="assets/icons/${icon}" alt=""><div><b>${a12UiEscape(title)}</b><span>${a12UiEscape(detail)}</span></div></div>`).join("")}<div class="a12-milestone-guide-note" data-testid="milestone-gayson-guide"><b>Gayson guide</b><span>Keep the open item open until the next typed control is verified; local fixture values never imply a live post or close.</span></div></div><aside class="a12-milestone-summary" aria-label="Settlement summary and decision states"><section class="a12-summary-panel" data-testid="milestone-settlement-summary"><h3>USDC · Arc settlement summary</h3><div class="a12-summary-token"><span class="a12-summary-brands"><img src="assets/brand/usdc-token-official.svg" alt="Official Circle USDC"><img src="assets/brand/arc-logo-navy-official.svg" alt="Official Arc"></span><div><span>Principal</span><b>${a12UiEscape(view.canvas.principal)}</b></div></div><div class="a12-summary-pairs"><div><span>Counterparty</span><b>${a12UiEscape(view.canvas.counterparty)}</b></div><div><span>Open item</span><b>${a12UiEscape(view.canvas.sourceDocument)}</b></div><div><span>Network</span><b>Arc Testnet</b></div><div><span>Status</span><b>${a12UiEscape(view.canvas.scenarioState)}</b></div></div></section><section class="a12-summary-panel" data-testid="milestone-decision-states"><h3>Decision states · recovery</h3><div class="a12-decision-row" data-a12-decision="current" data-state="current"><b>Current · ${a12UiEscape(view.canvas.scenarioState)}</b><span>${a12UiEscape(view.canvas.recovery)} · next owner: ${a12UiEscape(view.canvas.command.next_owner)}</span></div><div class="a12-decision-row" data-a12-decision="stale" data-state="held"><b>Stale evidence</b><span>Keep the business item open · next owner: reviewer · recovery: refresh the typed observation.</span></div><div class="a12-decision-row" data-a12-decision="mismatch" data-state="held"><b>Receipt mismatch</b><span>Keep the open item unchanged · next owner: reconciliation · recovery: inspect the first exact mismatch.</span></div></section></aside></div>`;
  }
...
export function mountA12DeepWorkbench(root = null, { initialHash = globalThis.location?.hash ?? "" } = {}) {
  ...
  host.addEventListener("click", (event) => {
    const target = event.target.closest("[data-a12-workspace],[data-a12-scenario],[data-a12-stage],[data-a12-tab],[data-a12-filter],[data-a12-primary],[data-a12-open-inspector],[data-a12-close-inspector],[data-a12-object],[data-a12-edit-select],[data-a12-origin-entry]");
