  ...
  canvas.addEventListener("click", (event) => {
    ...
    if (control.dataset.workflowAction) {
      ...
      if (action === "rank") {
        ...
        dispatch?.({ type: "SET_ALLOCATION", allocation: { amount6: state.settlementCase?.allocation?.requestedAmount6 ?? "1250000000", authority: { role: "operator", operatorId: "local-operator" }, originalReference: state.settlementCase?.allocation?.originalReference ?? null } });
        ...
        dispatch?.({ type: "CLOSE_OPERATIONAL" });
      } else if (action === "close-period") {
        dispatch?.({ type: "CLOSE_ACCOUNTING_PERIOD", periodStatus: "ended" });
      } else if (action === "close-business") {
        dispatch?.({ type: "CLOSE_BUSINESS" });
