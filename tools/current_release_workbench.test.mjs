...
test("current narrative and compiler contract reject historical umbrella drift", async () => {
  ...
  assert.match(readme, /^# Arc Enterprise Settlement & Cash Control/m);
  assert.match(readme, /\*\*Formal product name:\*\* Arc Enterprise Settlement & Cash Control/);
  ...
  assert.match(readme, /c29c19b1cc5ede7c2f2984f730991d410ca41812/);
  assert.match(readme, /\[`current-release-workbench-manifest\.json`\]\(current-mvp\/current-release-workbench-manifest\.json\)/);
  assert.doesNotMatch(readme, /675a9dcf55248efae8b1a845c023fe6b6e31201f6b026714e1041567b16dc834/);
  assert.match(readme, /existing Render service `srv-d9cumml8nd3s73c9nehg`/);
  assert.match(readme, /Encode requires a current-release rebind/);
  assert.match(readme, /Circle monitor state is `enabled=true`, but current Event History has `0` rows/);
  assert.match(readme, /no historical backfill and no current event receipt/);
  assert.match(readme, /ERP business close remains unproven/);
  assert.match(readme, /Final remains unproven/);
  assert.match(readme, /former `2524f0d` \/ `dep-d9ucvtdbedkc73a0lgn0` pair is retained only as historical publication lineage/);
  assert.doesNotMatch(readme, /Current product source:.*2524f0d/);
  assert.doesNotMatch(readme, /Current-release boundary:.*dep-d9ucvtdbedkc73a0lgn0/);
  assert.doesNotMatch(readme, /no active subscription ID and no trusted readback loader/);
