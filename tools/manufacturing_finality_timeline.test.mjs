...
import { buildManufacturingFinalityTimeline, buildManufacturingReplayGuard } from "./arc_payment_receipt_server.mjs";
...
test("blocks replay after a reconciled manufacture-completion anchor", () => {
  const result = buildManufacturingReplayGuard(progress, reconciliation);
  assert.equal(result.status, "terminal_state_replay_blocked");
  assert.equal(result.decision, "do_not_replay_prior_quality_release_or_manufacture_anchor");
  assert.equal(result.boundaries.wallet_or_chain_action, false);
