...
test("Arc Lab viewer renders the cross-system manufacturing reconciliation", async () => {
  ...
  assert.match(viewer, /\/api\/v1\/final-submission-readiness/);
  assert.match(viewer, /\/api\/v1\/final-demo-plan/);
  ...
  assert.match(viewer, /Reviewer Submission Handoff/);
  assert.match(viewer, /This is a navigation plan, not a recorded video/);
});
