...
**Current product source:** [GitHub `main`](https://github.com/gaysonloser/arc-payment-receipt/tree/main) contains the accepted product implementation. The current Arc-receipt implementation commit is [`e27d847`](https://github.com/gaysonloser/arc-payment-receipt/commit/e27d847454a4bf2c05319c9a3e2a7c1fc6976c4d); it refines the settlement workbench interaction while keeping every absent receipt fail-closed.

**Published demo:** [arc-payment-receipt.onrender.com/current-mvp/](https://arc-payment-receipt.onrender.com/current-mvp/) is the existing Render service. Its current immutable deployment receipt is the older [`83becbb`](https://github.com/gaysonloser/arc-payment-receipt/commit/83becbb58bb88be12cabf129af784db79747e958); the public service must not be described as serving `e27d847` until the existing Render deployment/readback owner gate is consumed. The `e27d847` candidate is `NOT_DEPLOYED` with owner gate `NOT_GRANTED`. It runs as a free Render web service and may take 50 seconds or more to wake after inactivity.

**Current-release boundary:** GitHub `main@e27d847` is the current source receipt, while Render still serves the older immutable `83becbb` receipt; the candidate `e27d847` remains `NOT_DEPLOYED` pending its existing owner gate. It carries a current-contract Arc Testnet readback for `PolicySettlementV1`: deployment code, one successful `createPolicy` transaction, its `PolicyCreated` log, and an independently matching `getPolicy` result. Public current-release binding remains pending until Render independently reads back the requested commit. Arc Testnet facts and owner-live ERP readbacks remain separate evidence classes and do not imply settlement execution, ERP posting or business close. Circle Console remains fail-closed without a current subscription/readback; ERP posting/business close, Encode and Final remain unproven. See [Cloud Runtime And Delivery Surfaces](docs/CLOUD_RUNTIME.md).
...
- Current product implementation: [`e27d847454a4bf2c05319c9a3e2a7c1fc6976c4d`](https://github.com/gaysonloser/arc-payment-receipt/commit/e27d847454a4bf2c05319c9a3e2a7c1fc6976c4d), published to `main`; Render deployment/readback for this commit remains owner-gated.
...
- Published runtime: [Render `/current-mvp/`](https://arc-payment-receipt.onrender.com/current-mvp/); current immutable deployment `83becbb`. The `e27d847` candidate is `NOT_DEPLOYED` until the owner-authorized Render deployment/readback gate is completed.
...
These receipts belong to the retained `ArcPaymentReceipt` and Arc Lab lineage. They do not supply the current `PolicySettlementV1` proof. The current `e27d847` source carries a source-separated current-contract receipt: [`createPolicy` transaction `0x2f40…a9c`](https://testnet.arcscan.app/tx/0x2f40fa6b8d464fd2b35a34612ee2e90dbb4121b3a2ddfad652505599b2ed4a9c), its `PolicyCreated` log, and an independently matching `getPolicy` readback; public Render release binding remains pending until the owner-authorized deployment/readback gate is completed.
