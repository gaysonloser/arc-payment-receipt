# Arc Enterprise Settlement & Cash Control

<img src="assets/payment-receipt-logo.png" width="120" alt="Verified Milestone Close logo">

**Formal product name:** Arc Enterprise Settlement & Cash Control. Verified Milestone Close is the current release: a receipt-first Arc/USDC workbench for a treasury operator reviewing a supplier payable and its customer-receipt/refund counterpart. `PolicySettlementV1` supplies the programmable-money boundary; the workbench separately displays typed ERPNext Invoice, Payment Entry, Bank Transaction, GL, Payment Ledger, Accounting Period and close readbacks. Chain success never implies ERP posting or business close.

**Current product source:** [GitHub `main`](https://github.com/gaysonloser/arc-payment-receipt/tree/main) contains the accepted product implementation at [`c29c19b1cc5ede7c2f2984f730991d410ca41812`](https://github.com/gaysonloser/arc-payment-receipt/commit/c29c19b1cc5ede7c2f2984f730991d410ca41812). The current public content manifest is [`current-release-workbench-manifest.json`](current-mvp/current-release-workbench-manifest.json); the public route and manifest bytes are verified against `main@c29c19b1cc5ede7c2f2984f730991d410ca41812` without embedding a self-referential manifest digest here.

**Published demo:** [arc-payment-receipt.onrender.com/current-mvp/](https://arc-payment-receipt.onrender.com/current-mvp/) is the existing Render service `srv-d9cumml8nd3s73c9nehg`. The public route and manifest both currently return HTTP 200 and the public manifest matches `main@c29c19b1cc5ede7c2f2984f730991d410ca41812`. The public route does not expose an immutable deployment entity or deployment ID; that binding remains an external Render dashboard receipt and is not guessed or replaced with a historical ID. It runs as a free Render web service and may take 50 seconds or more to wake after inactivity.

**Current-release boundary:** GitHub `main@c29c19b1cc5ede7c2f2984f730991d410ca41812` and the public Render route are the current source/runtime surfaces; an immutable Render deployment ID still requires the external dashboard receipt described above. The source carries a current-contract Arc Testnet readback for `PolicySettlementV1`: deployment code, one successful `createPolicy` transaction, its `PolicyCreated` log, and an independently matching `getPolicy` result. Arc Testnet facts and owner-live ERP readbacks remain separate evidence classes and do not imply settlement execution, ERP posting or business close. Encode requires a current-release rebind because its last accepted authenticated binding predates c29c19b1cc5ede7c2f2984f730991d410ca41812. Circle monitor state is `enabled=true`, but current Event History has `0` rows with no historical backfill and no current event receipt, so Circle Console remains fail-closed and blocked; ERP business close remains unproven, and Final remains unproven. See [Cloud Runtime And Delivery Surfaces](docs/CLOUD_RUNTIME.md).
...
- Current product implementation: [`c29c19b1cc5ede7c2f2984f730991d410ca41812`](https://github.com/gaysonloser/arc-payment-receipt/commit/c29c19b1cc5ede7c2f2984f730991d410ca41812), published to `main`; public Render `/current-mvp/` and manifest readbacks are HTTP 200 and match this commit. Immutable deployment ID remains unasserted until the external Render dashboard receipt is captured.
...
- Published runtime: [Render `/current-mvp/`](https://arc-payment-receipt.onrender.com/current-mvp/); current public route matches GitHub `main@c29c19b1cc5ede7c2f2984f730991d410ca41812`, while the immutable deployment entity remains an external receipt not exposed by the public route.
...
These receipts belong to the retained `ArcPaymentReceipt` and Arc Lab lineage. They do not supply the current `PolicySettlementV1` proof. The current `c29c19b1cc5ede7c2f2984f730991d410ca41812` source carries a source-separated current-contract receipt: [`createPolicy` transaction `0x2f40…a9c`](https://testnet.arcscan.app/tx/0x2f40fa6b8d464fd2b35a34612ee2e90dbb4121b3a2ddfad652505599b2ed4a9c), its `PolicyCreated` log, and an independently matching `getPolicy` readback. The former `2524f0d` / `dep-d9ucvtdbedkc73a0lgn0` pair is retained only as historical publication lineage and is not the current release.
...
- Circle monitor state: H218 readback records `enabled=true`, but current Event History has `0` rows, with no historical backfill and no current event receipt. Historical `PaymentReceived / Subscribed` evidence remains source-separated; the current `ArcEnterpriseEvidenceRegistryV2` is import-only and Circle Console remains fail-closed and blocked.
